...
// use flatten to simplify to reduce number of final leaf nodes and return
...
function parse(xml) {
  ...
  // use flatten(data) to simplify to reduce number of final leaf nodes and return
  return data;
}

const parseXML = xml => {
  var json = new DOMParser().parseFromString(xml, 'text/xml');
  return parse(json);
};
